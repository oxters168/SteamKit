...
namespace SteamKit2
{
    ...
    public static class CryptoHelper
    {
        ...
        /// <summary>
        /// Decrypts using AES/CBC/PKCS7 with an input stream and key, using the random IV prepended using AES/ECB/None
        /// </summary>
        public static Stream SymmetricDecrypt( Stream input, byte[] key )
        {
            if ( input == null )
            {
                throw new ArgumentNullException( nameof( input ) );
        ...
        /// Decrypts using AES/CBC/PKCS7 with an input stream and key, using the random IV prepended using AES/ECB/None
        /// </summary>
        static Stream SymmetricDecrypt( Stream input, byte[] key, out byte[] iv )
        {
            if ( input == null )
            {
                throw new ArgumentNullException( nameof( input ) );
            ...
            using ( var aes = Aes.Create() )
            {
                ...
                using ( var aesTransform = aes.CreateDecryptor( key, iv ) )
                //using ( var ms = new MemoryStream( cipherText ) )
                using ( var cs = new CryptoStream( input, aesTransform, CryptoStreamMode.Read ) )
                {
                    // plaintext is never longer than ciphertext
                    //byte[] plaintext = new byte[ cipherText.Length ];
        ...
        /// Performs an Adler32 on the given input
        /// </summary>
        public static byte[] AdlerHash( Stream input )
        {
            if ( input == null )
            {
                throw new ArgumentNullException( nameof( input ) );
